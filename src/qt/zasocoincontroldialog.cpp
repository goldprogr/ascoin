...
#include "ui_zasocoincontroldialog.h"
...
std::list<std::string> ZasocoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZasocoinControlDialog::listMints;

ZasocoinControlDialog::ZasocoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZasocoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZasocoinControlDialog::~ZasocoinControlDialog()
{
    delete ui;
}

void ZasocoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZasocoinControlDialog::updateList()
{
...
void ZasocoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZasocoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZasocoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZasocoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZasocoinControlDialog::GetSelectedMints()
{
...
void ZasocoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
