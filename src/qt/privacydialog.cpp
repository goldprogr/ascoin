...
#include "zasocoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->labelzCDMSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCDMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCDM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
...
void PrivacyDialog::on_pushButtonSpendzCDM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCDM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCDM
        sendzCDM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCDM
    sendzCDM();
}

void PrivacyDialog::on_pushButtonZasocoinControl_clicked()
{
    ZasocoinControlDialog* zCDMControl = new ZasocoinControlDialog(this);
    zCDMControl->setModel(walletModel);
    zCDMControl->exec();
}

void PrivacyDialog::setZasocoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCDMSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCDM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid asocoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCDMpayAmount->text().toDouble();
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " asocoin </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZasocoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZasocoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zasocoin selector in case it was used
    ZasocoinControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " asocoin, ";
...
{
    ui->labelzCDMSyncStatus->setVisible(fShow);
}
